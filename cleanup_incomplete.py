#!/usr/bin/env python3
"""
Cleanup Script for Incomplete Video Processing

This script safely cleans up partial results from a video that didn't complete
due to timeout or crash. Use before restarting processing.

Usage:
    python cleanup_incomplete.py --video_index 17
    python cleanup_incomplete.py --video_index 17 --dry-run  # See what would be deleted
    python cleanup_incomplete.py --video_index 17 --force    # Delete even if merge exists
"""
...
parser = argparse.ArgumentParser(description="Clean up incomplete video processing results")
...
print(f"🧹 Cleanup for Video Index {args.video_index}")
...
# File patterns
partial_pattern = os.path.join(PROJECT_DIR, f"partial_results_qwen7b_vid{args.video_index}_gpu*.json")
merged_file = os.path.join(PROJECT_DIR, f"final_predictions_qwen7b_video_{args.video_index}.json")
...
if merged_exists and not args.force:
    ...
    print()
    print("   Options:")
    print("   1. If video completed: No cleanup needed, skip to next video")
    print("   2. If partial/corrupted: Use --force to delete and reprocess")
    print("   3. Check the file first:")
    print(f"      python -c \"import json; d=json.load(open('{merged_file}')); print(f'Events: {{len(d)}}')\"")
    print()
    exit(0)
