#!/usr/bin/env python3
"""
Cleanup Script for Incomplete LLaVA Video Processing

Usage:
    python cleanup_llava.py --video_index 5
    python cleanup_llava.py --video_index 5 --dry-run
    python cleanup_llava.py --video_index 5 --force
"""
...
parser = argparse.ArgumentParser(description="Clean up incomplete LLaVA video processing results")
...
print(f"🧹 Cleanup for LLaVA Video Index {args.video_index}")
...
# File patterns
partial_pattern = os.path.join(PROJECT_DIR, f"partial_results_llava_vid{args.video_index}_gpu*.json")
merged_file = os.path.join(PROJECT_DIR, f"final_predictions_llava_video_{args.video_index}.json")
