#!/bin/bash
#SBATCH --job-name=SoccerLLaVA
#SBATCH --partition=GPU
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=1
#SBATCH --cpus-per-task=8
#SBATCH --mem=64G
#SBATCH --time=06:00:00            # 6 hours for safety
#SBATCH --gres=gpu:TitanRTX:4
#SBATCH --output=llava_%j.log
#SBATCH --exclusive

# ============================================================================
# LLaVA-NeXT-Video Batch Processing Script
# Usage: sbatch submit_llava.sh [START_VIDEO] [END_VIDEO]
#
# Examples:
#   sbatch submit_llava.sh 0 39     # Process all videos
#   sbatch submit_llava.sh 0 10     # Process first 10 videos
#   sbatch submit_llava.sh 20 39    # Process videos 20-39
# ============================================================================
...
echo "=========================================="
echo "🎬 LLaVA-NeXT-Video BATCH PROCESSING"
echo "=========================================="
echo "Model: LLaVA-NeXT-Video-7B-hf"
...
do
    ...
    MERGED_FILE="$HOME/soccer_project/final_predictions_llava_video_${i}.json"
    ...
    PARTIAL_COUNT=$(ls $HOME/soccer_project/partial_results_llava_vid${i}_gpu*.json 2>/dev/null | wc -l)
    if [ $PARTIAL_COUNT -gt 0 ]; then
        echo "⚠️ Found $PARTIAL_COUNT partial file(s) from previous run - cleaning up..."
        rm $HOME/soccer_project/partial_results_llava_vid${i}_gpu*.json
        echo "   ✓ Cleaned"
    fi

    # Launch GPU swarm for this video
    echo "🚀 Launching 4-GPU LLaVA swarm..."
    python -u $HOME/soccer_project/llava_split_worker.py --gpu_id 0 --num_workers 4 --video_index $i &
    python -u $HOME/soccer_project/llava_split_worker.py --gpu_id 1 --num_workers 4 --video_index $i &
    python -u $HOME/soccer_project/llava_split_worker.py --gpu_id 2 --num_workers 4 --video_index $i &
    python -u $HOME/soccer_project/llava_split_worker.py --gpu_id 3 --num_workers 4 --video_index $i &
    ...
    python $HOME/soccer_project/merge_results_llava.py --video_index $i
    ...
    if [ -f "$MERGED_FILE" ]; then
        ...
        if [ "$EVENT_COUNT" -gt 0 ]; then
            ...
            rm $HOME/soccer_project/partial_results_llava_vid${i}_gpu*.json 2>/dev/null
...
echo "=========================================="
echo "🎉 LLAVA BATCH PROCESSING COMPLETE"
echo "=========================================="
