#!/bin/bash
#SBATCH --job-name=SoccerIdefics2
...
#SBATCH --output=idefics2_%j.log
#SBATCH --exclusive

# ============================================================================
# Idefics2-8B Batch Processing Script
# Usage: sbatch submit_idefics2.sh [START_VIDEO] [END_VIDEO]
#
# Examples:
#   sbatch submit_idefics2.sh 0 39     # Process all videos
#   sbatch submit_idefics2.sh 0 10     # Process first 10 videos
#   sbatch submit_idefics2.sh 20 39    # Process videos 20-39
# ============================================================================
...
echo "=========================================="
echo "🎬 IDEFICS2-8B BATCH PROCESSING"
echo "=========================================="
echo "Model: HuggingFaceM4/idefics2-8b"
...
do
    ...
    MERGED_FILE="$HOME/soccer_project/final_predictions_idefics2_video_${i}.json"
    ...
    PARTIAL_COUNT=$(ls $HOME/soccer_project/partial_results_idefics2_vid${i}_gpu*.json 2>/dev/null | wc -l)
    if [ $PARTIAL_COUNT -gt 0 ]; then
        echo "⚠️ Found $PARTIAL_COUNT partial file(s) from previous run - cleaning up..."
        rm $HOME/soccer_project/partial_results_idefics2_vid${i}_gpu*.json
        echo "   ✓ Cleaned"
    fi

    # Launch GPU swarm for this video
    echo "🚀 Launching 4-GPU Idefics2 swarm..."
    python -u $HOME/soccer_project/idefics2_split_worker.py --gpu_id 0 --num_workers 4 --video_index $i &
    python -u $HOME/soccer_project/idefics2_split_worker.py --gpu_id 1 --num_workers 4 --video_index $i &
    python -u $HOME/soccer_project/idefics2_split_worker.py --gpu_id 2 --num_workers 4 --video_index $i &
    python -u $HOME/soccer_project/idefics2_split_worker.py --gpu_id 3 --num_workers 4 --video_index $i &
    ...
    python $HOME/soccer_project/merge_results_idefics2.py --video_index $i
    ...
    if [ -f "$MERGED_FILE" ]; then
        ...
        if [ "$EVENT_COUNT" -gt 0 ]; then
            ...
            rm $HOME/soccer_project/partial_results_idefics2_vid${i}_gpu*.json 2>/dev/null
...
echo "=========================================="
echo "🎉 IDEFICS2 BATCH PROCESSING COMPLETE"
echo "=========================================="
